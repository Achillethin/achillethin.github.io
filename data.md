---
layout: page
title: Data
show-avatar: True
---

Here you can find the data available for the teaching projects:

- [eucalyptus.txt](docs/data/eucalyptus.txt)
- [mais.txt](docs/data/mais.txt)
- [ozone.txt](docs/data/ozone.txt)
- [source_wagesmicrodata.xls](docs/data/source_wagesmicrodata.xls)
- [toy_example.csv](docs/data/mais.txt)
- [wagesmicrodata.csv](docs/data/wagesmicrodata.csv)
- [wagesmicrodata.xls](docs/data/wagesmicrodata.xls)

### Slides 
- [Introduction au modèle linéaire](docs/slides/MLG_intro.pdf)
- [Régression linéaire simple](docs/slides/MLG_chapter1.pdf)
- [Régression linéaire multiple](docs/slides/MLG_chapter2.pdf)
- [Analyse de la variance à un facteur](docs/slides/Anova_1facteur.pdf)
- [Analyse de la covariance](docs/slides/MLG_ANCOVA.pdf)
- [Autres méthodes de sélection de variables](docs/slides/MLG_chap5.pdf)


### Project
- [Project EMSBD 2023](docs/slides/MLG_project.pdf)
- [Infos sur le jeu de données cars](docs/data/cars_info.xlsx)
- [cars_train.csv](docs/data/cars_train.csv)
- [cars_test.csv](docs/data/cars_test.csv)
- [Vins rouges](docs/data/wineQualityReds.csv)
- [Vins blancs](docs/data/wineQualityWhites.csv)
<!---
-->


### Code

- [Introduction au modèle linéaire](docs/code/MLG_intro.R)
- [Régression linéaire simple](docs/code/MLG_chap1.R)
- [Régression linéaire multiple](docs/code/MLG_chap2.R)
- [Analyse de la variance à un facteur](docs/code/MLG_chap3_1F.R)
- [Analyse de la variance à deux facteurs](docs/code/MLG_chap3_2F.R)
- [Analyse de la covariance](docs/code/MLG_chap4.R)


<!---
- [Régression linéaire multiple](docs/slides/MLG_chapter2.pdf)
-->
